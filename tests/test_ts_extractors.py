...
class TestTsNamingExtraction:
    def test_python_snake_case_functions(self) -> None:
        source = "def get_user():\n    pass\n\ndef set_name():\n    pass\n"
        patterns = ts_extract_naming(_parse(source, Language.PYTHON, FAKE_PATH), Language.PYTHON, FAKE_PATH)
    ...
    def test_typescript_camel_case_functions(self) -> None:
        ...
        patterns = ts_extract_naming(_parse(source, Language.TYPESCRIPT, Path("test.ts")), Language.TYPESCRIPT, Path("test.ts"))
    ...
    def test_go_functions_and_structs(self) -> None:
        ...
        patterns = ts_extract_naming(_parse(source, Language.GO, Path("main.go")), Language.GO, Path("main.go"))
        names = {p.name for p in patterns}
        assert "function_naming" in names
    ...
    def test_rust_functions_and_structs(self) -> None:
        ...
        patterns = ts_extract_naming(_parse(source, Language.RUST, Path("lib.rs")), Language.RUST, Path("lib.rs"))
        names = {p.name for p in patterns}
        assert "function_naming" in names
        assert "class_naming" in names
    ...
    def test_java_methods_and_classes(self) -> None:
        ...
        patterns = ts_extract_naming(_parse(source, Language.JAVA, Path("UserService.java")), Language.JAVA, Path("UserService.java"))
        names = {p.name for p in patterns}
        assert "function_naming" in names
        assert "class_naming" in names
...
class TestTsErrorHandling:
    def test_python_except_types(self) -> None:
        source = "try:\n    x = 1\nexcept ValueError:\n    pass\nexcept OSError:\n    pass\n"
        patterns = ts_extract_error_handling(_parse(source, Language.PYTHON, FAKE_PATH), Language.PYTHON, FAKE_PATH)
    ...
    def test_go_err_nil_check(self) -> None:
        ...
        patterns = ts_extract_error_handling(_parse(source, Language.GO, Path("main.go")), Language.GO, Path("main.go"))
    ...
    def test_rust_result_and_question_mark(self) -> None:
        ...
        patterns = ts_extract_error_handling(_parse(source, Language.RUST, Path("lib.rs")), Language.RUST, Path("lib.rs"))
        names = {p.name for p in patterns}
        assert "result_type" in names

    def test_js_try_catch(self) -> None:
        source = "try {\n  doStuff();\n} catch (e) {\n  console.error(e);\n}\n"
        patterns = ts_extract_error_handling(_parse(source, Language.JAVASCRIPT, Path("app.js")), Language.JAVASCRIPT, Path("app.js"))
...
class TestTsTestingPatterns:
    def test_pytest_detection(self) -> None:
        ...
        patterns = ts_extract_testing(_parse(source, Language.PYTHON, Path("test_main.py")), source, Language.PYTHON, Path("test_main.py"))
        names = {p.name for p in patterns}
    ...
    def test_pytest_fixture(self) -> None:
        ...
        patterns = ts_extract_testing(_parse(source, Language.PYTHON, Path("test_fixtures.py")), source, Language.PYTHON, Path("test_fixtures.py"))
        names = {p.name for p in patterns}
    ...
    def test_go_table_driven(self) -> None:
        ...
        patterns = ts_extract_testing(_parse(source, Language.GO, Path("add_test.go")), source, Language.GO, Path("add_test.go"))
        names = {p.name for p in patterns}
        assert "test_framework" in names
    ...
    def test_jest_detection(self) -> None:
        ...
        patterns = ts_extract_testing(_parse(source, Language.JAVASCRIPT, Path("math.test.js")), source, Language.JAVASCRIPT, Path("math.test.js"))
        names = {p.name for p in patterns}
        assert "test_framework" in names
    ...
    def test_non_test_file_returns_empty(self) -> None:
        source = "def helper():\n    return 42\n"
        patterns = ts_extract_testing(_parse(source, Language.PYTHON, Path("utils.py")), source, Language.PYTHON, Path("utils.py"))
        assert patterns == []
...
class TestTsImports:
    def test_python_absolute_imports(self) -> None:
        ...
        patterns = ts_extract_imports(_parse(source, Language.PYTHON, FAKE_PATH), Language.PYTHON, FAKE_PATH)
    ...
    def test_go_import_grouping(self) -> None:
        ...
        patterns = ts_extract_imports(_parse(source, Language.GO, Path("main.go")), Language.GO, Path("main.go"))
...
class TestTsDocumentation:
    def test_python_google_style_docstrings(self) -> None:
        ...
        patterns = ts_extract_documentation(_parse(source, Language.PYTHON, FAKE_PATH), Language.PYTHON, FAKE_PATH)
        names = {p.name for p in patterns}
    ...
    def test_jsdoc_detection(self) -> None:
        ...
        patterns = ts_extract_documentation(_parse(source, Language.JAVASCRIPT, Path("api.js")), Language.JAVASCRIPT, Path("api.js"))
...
class TestTsStyle:
    def test_python_type_hints(self) -> None:
        ...
        patterns = ts_extract_style(_parse(source, Language.PYTHON, FAKE_PATH), source, Language.PYTHON, FAKE_PATH)
    ...
    def test_js_semicolons(self) -> None:
        ...
        patterns = ts_extract_style(_parse(source, Language.JAVASCRIPT, Path("app.js")), source, Language.JAVASCRIPT, Path("app.js"))
...
class TestTsLogging:
    def test_python_standard_logging(self) -> None:
        ...
        patterns = ts_extract_logging(_parse(source, Language.PYTHON, FAKE_PATH), Language.PYTHON, FAKE_PATH)
        names = {p.name for p in patterns}
    ...
    def test_js_console_logging(self) -> None:
        ...
        patterns = ts_extract_logging(_parse(source, Language.JAVASCRIPT, Path("app.js")), Language.JAVASCRIPT, Path("app.js"))
...
class TestCompatibility:
    ...
    def test_pattern_categories_are_valid(self) -> None:
        ...
        all_patterns = []
        all_patterns.extend(ts_extract_naming(_parse(source, Language.PYTHON, FAKE_PATH), Language.PYTHON, FAKE_PATH))
        all_patterns.extend(ts_extract_error_handling(_parse(source, Language.PYTHON, FAKE_PATH), Language.PYTHON, FAKE_PATH))
        all_patterns.extend(ts_extract_imports(_parse(source, Language.PYTHON, FAKE_PATH), Language.PYTHON, FAKE_PATH))
        all_patterns.extend(ts_extract_documentation(_parse(source, Language.PYTHON, FAKE_PATH), Language.PYTHON, FAKE_PATH))
        all_patterns.extend(ts_extract_style(_parse(source, Language.PYTHON, FAKE_PATH), source, Language.PYTHON, FAKE_PATH))
        all_patterns.extend(ts_extract_logging(_parse(source, Language.PYTHON, FAKE_PATH), Language.PYTHON, FAKE_PATH))
